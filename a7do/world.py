...
class WorldMap:
    ...
    streets: List[Tuple[str, str]] = field(default_factory=list)  # place_id -> place_id connections
...
def generate_world(seed: int, neighbour_count: int = 20) -> WorldMap:
    ...
    # --- Anchor: Hospital at exact centre (0,0,0)
    hospital = Place(
    ...
    # Neighbours along Home Street (door numbers 2..(neighbour_count+1))
    # Place them in a simple line offset around home
    for i in range(1, neighbour_count + 1):
        door_no = str(i + 1)
        x = 28.0 + (i * 2.0)  # march along x
    ...
    # --- Street connections (schematic)
    # hospital -> park -> shops -> home_street -> a7do_home
    ...
    # Connect neighbours to home street and to each other lightly
...
def ascii_map(world: WorldMap) -> str:
    """
    Simple schematic listing (not a true grid renderer).
    Useful for Observer.
    """
    lines = []
    ...
    for a, b in world.streets[:40]:
        lines.append(f"- {a} -> {b}")
    if len(world.streets) > 40:
        lines.append(f"... {len(world.streets)-40} more")
    return "\n".join(lines)